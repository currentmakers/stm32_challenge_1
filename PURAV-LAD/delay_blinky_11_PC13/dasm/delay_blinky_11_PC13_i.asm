...
__attribute__((naked)) int Reset_Handler(void)  {
 ...
 8000008:	2401      	movs	r4, #1
    ...
    *(PRPH_ALIAS_ADDR(AHB1ENR_ADDR, 2)) = Counter;
 800000a:	4b04      	ldr	r3, [pc, #16]	; (800001c <Reset_Handler+0x14>)
 800000c:	601c      	str	r4, [r3, #0]

    // assign address of GPIOC MODER, and set LED To output
    // Also, no need to reset bit 27, as it is zero at reset state
    // Also, we will make reuse of delay counter
    *(PRPH_ALIAS_ADDR(GPIOC_MODER_ADDR, 2 * LED_PC13)) = Counter;
 800000e:	4b04      	ldr	r3, [pc, #16]	; (8000020 <Reset_Handler+0x18>)
 8000010:	601c      	str	r4, [r3, #0]

    // we will cache the addresses for IO.
    RegToReadWrite = PRPH_ALIAS_ADDR(GPIOC_ODR_ADDR, LED_PC13);
 ...
 800001c:	42470608 	.word	0x42470608
 8000020:	42410068 	.word	0x42410068
 8000024:	424102b4 	.word	0x424102b4
